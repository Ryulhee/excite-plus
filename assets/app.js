...
(function () {
  ...
  function commonInit() {
    ...
    $$('[data-href]').forEach(a => {
      if (a.dataset.href === page) a.classList.add('active');
    });
  ...
  function initVitalUpload() {
    ...
    [selectBtn, downloadFeatures, downloadDict].forEach(removeDisabledAction);
    ...
    fileInput?.addEventListener('change', () => processAutoCsvFiles(fileInput.files));
    ...
    dropzone?.addEventListener('drop', (e) => {
      ...
      processAutoCsvFiles(e.dataTransfer.files);
    });

    downloadFeatures?.addEventListener('click', () => {
      if (!latestResult?.features?.length) return toast('먼저 CSV를 업로드해 자동 변환을 완료하세요.', 'gray');
    ...
    downloadDict?.addEventListener('click', () => {
      if (!latestResult?.dictionary?.length) return toast('먼저 CSV를 업로드해 자동 변환을 완료하세요.', 'gray');
    ...
    async function processAutoCsvFiles(fileList) {
      ...
      if (rejected.length) {
        toast('.vital 포함 비CSV 파일은 이 화면에서 업로드할 수 없습니다. raw CSV만 선택하세요.', 'yellow');
      }
      if (!csvFiles.length) {
        setStatusLocal('CSV 파일만 업로드할 수 있습니다. .vital 파일은 차단되었습니다.');
        return;
      }

      setMetricText('#autoCsvTotal', `${csvFiles.length}개`, '처리 중');
      setMetricText('#autoCsvConverted', '0개', '진행');
      setMetricText('#autoCsvQcDone', '0개', '진행');
      setMetricText('#autoCsvError', `${rejected.length}개`, rejected.length ? '확인' : '대기');
      setProgressLocal(12);
      ...
      for (let i = 0; i < csvFiles.length; i++) {
        ...
        setProgressLocal(15 + Math.round((i / csvFiles.length) * 65));
        ...
        try {
          ...
          const resultKey = `csv_${Date.now()}_${i}_${Math.random().toString(16).slice(2)}`;
          ...
          patchState(s => {
            const duplicateIndex = s.vitalFiles.findIndex(v => v.name === file.name && v.size === file.size);
            const item = {
              id: duplicateIndex >= 0 ? s.vitalFiles[duplicateIndex].id : uid('CSVF'),
              caseId: result.caseId,
              name: file.name,
              size: file.size,
              sourceType: 'raw CSV',
              ...
              previewVariables: ['HR','MAP','SBP','DBP','SpO2','PI','RR'],
              previewData: null,
              tagIds: duplicateIndex >= 0 ? (s.vitalFiles[duplicateIndex].tagIds || []) : [],
              patientId: duplicateIndex >= 0 ? (s.vitalFiles[duplicateIndex].patientId || '') : '',
              eventId: duplicateIndex >= 0 ? (s.vitalFiles[duplicateIndex].eventId || '') : ''
            };
            if (duplicateIndex >= 0) s.vitalFiles[duplicateIndex] = item;
            else s.vitalFiles.unshift(item);
          ...
          addAudit('raw csv auto conversion', 'raw_csv', `${file.name} -> ${result.outputName}`);
        } catch (error) {
          console.error(error);
          errors += 1;
          rowsForTable.push({
            name: file.name,
            size: file.size,
            caseId: makeCaseIdFromName(file.name, true),
            status: '변환 오류',
            featureRows: '-',
            segmentCount: '-',
            qc: '불가',
    ...
    function renderAutoCsvUploadRows(rows) {
      if (!body) return;
      if (!rows.length) {
        setTableEmpty(body, 8, '아직 업로드된 CSV가 없습니다.', 'CSV 파일을 업로드하면 자동으로 변환 및 QC가 실행됩니다.');
        return;
      }
      body.innerHTML = rows.map(row => {
        const isSelectable = row.resultKey && conversionResults.has(row.resultKey);
        const selected = isSelectable && row.resultKey === selectedResultKey;
        const trClass = [isSelectable ? 'qc-selectable-row' : '', selected ? 'selected' : ''].filter(Boolean).join(' ');
        const dataAttr = isSelectable ? ` data-auto-csv-result="${escapeHtml(row.resultKey)}" title="클릭하면 이 파일의 신호 궤적 QC를 위에 표시합니다."` : '';
        return `<tr class="${trClass}"${dataAttr}>
          <td>${escapeHtml(row.name)}${isSelectable ? ' <span class="badge gray qc-row-hint">QC 보기</span>' : ''}</td>
          <td>${formatBytes(row.size)}</td>
          <td>${escapeHtml(row.caseId)}</td>
          <td>${badge(row.status, row.status.includes('완료') ? 'green' : row.status.includes('오류') || row.status.includes('차단') ? 'red' : 'yellow')}</td>
          <td>${escapeHtml(row.featureRows)}</td>
          <td>${escapeHtml(row.segmentCount)}</td>
          <td>${badge(row.qc, row.qc.includes('완료') ? 'teal' : 'gray')}</td>
          <td>${escapeHtml(row.error)}</td>
        </tr>`;
      }).join('');
      $$('[data-auto-csv-result]', body).forEach(rowEl => {
        rowEl.addEventListener('click', () => {
          const key = rowEl.dataset.autoCsvResult;
          ...
          renderAutoCsvQc(result, result.sourceFile || result.caseId, { scroll: true });
        });
      });
    }
    ...
    function renderAutoCsvQc(result, fileName, options = {}) {
      ...
      if (qcSummary) {
        qcSummary.innerHTML = `<dl class="kv"><dt>선택 파일</dt><dd>${escapeHtml(fileName)}</dd><dt>case_id</dt><dd>${escapeHtml(result.caseId)}</dd><dt>feature row</dt><dd>${result.features.length.toLocaleString()}행</dd><dt>segment</dt><dd>${result.segmentCount.toLocaleString()}개</dd><dt>컬럼</dt><dd>${result.columns.length.toLocaleString()}개</dd><dt>감지 신호</dt><dd>${escapeHtml(detectedText)}</dd><dt>encoding</dt><dd>${escapeHtml(result.encoding)}</dd></dl>`;
      }
      const subtitle = core.length
        ? 'core vital trend를 기준으로 QC 궤적을 표시합니다.'
        : 'HR/MAP/SpO2/RR trend가 없어 Solar8000의 수치형 auxiliary channel을 QC 궤적으로 표시합니다.';
      renderSignalTrajectoryPanel(qcPanel, result.features, result.caseId, { title: '신호 궤적 QC', subtitle, badge: '자동 QC 완료', signals: qcSignals });
      if (options.scroll) $('#autoCsvQcCard')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    renderSignalTrajectoryPanel(qcPanel, [], '', { title: '신호 궤적 QC', subtitle: 'CSV 업로드 후 자동 변환되면 core vital 또는 감지된 수치형 channel 궤적이 표시됩니다.', emptyText: '아직 업로드된 CSV가 없습니다.', badge: '대기' });
  }
  ...
  function savePatientMaster() {
    const card = cardByHeading('고정 환자 정보 등록');
    if (!card) return;
    const inputs = $$('input', card);
    const selects = $$('select', card);
    const registrationNo = inputs[0]?.value.trim();
    const pseudoId = inputs[1]?.value.trim() || (registrationNo ? `P${registrationNo.slice(-4)}` : '');
    if (!registrationNo && !pseudoId) return toast('등록번호 또는 가명 연구대상자 ID를 입력하세요.', 'gray');
    const patient = {
      id: uid('PAT'), registrationNo, pseudoId,
      sex: selects[0]?.value || '', birthYear: inputs[2]?.value.trim() || '',
      height: inputs[3]?.value.trim() || '', weight: inputs[4]?.value.trim() || '',
      note: $('textarea', card)?.value.trim() || '', createdAt: nowText()
    };
    patchState(s => {
      const exists = s.patients.find(p => (registrationNo && p.registrationNo === registrationNo) || (pseudoId && p.pseudoId === pseudoId));
      if (exists) Object.assign(exists, patient, { id: exists.id, updatedAt: nowText() });
      else s.patients.unshift(patient);
    });
    $$('input, textarea', card).forEach(el => el.value = '');
    renderPatientMaster();
    renderStudyPatientAssignment();
    addAudit('patient master save', 'patient', pseudoId || registrationNo);
    toast('고정 환자 정보를 저장했습니다.');
  }

  function renderPatientMaster(state = readState()) {
    const body = $('#patientMasterBody');
    if (!body) return;
    if (!state.patients.length) return setTableEmpty(body, 7, '아직 등록된 환자 정보가 없습니다.', '환자 번호/가명 ID는 연구와 관계없이 고정되는 master 정보입니다.');
    body.innerHTML = state.patients.map(p => `<tr><td>${escapeHtml(p.pseudoId || '-')}</td><td>${escapeHtml(p.registrationNo ? '****' + p.registrationNo.slice(-4) : '-')}</td><td>${escapeHtml(p.sex || '-')}</td><td>${escapeHtml(p.birthYear || '-')}</td><td>${escapeHtml(p.height || '-')}</td><td>${escapeHtml(p.weight || '-')}</td><td><button class="btn ghost" data-delete-patient="${escapeHtml(p.id)}">삭제</button></td></tr>`).join('');
    $$('[data-delete-patient]', body).forEach(btn => btn.addEventListener('click', () => {
      patchState(s => { s.patients = s.patients.filter(p => p.id !== btn.dataset.deletePatient); s.studyPatients = s.studyPatients.filter(x => x.patientId !== btn.dataset.deletePatient); });
      renderPatientMaster(); renderStudyPatientAssignment(); toast('환자 master 정보를 삭제했습니다.');
    }));
    setMetricByLabel('등록된 환자 master', `${state.patients.length}명`, state.patients.length ? '등록됨' : '대기');
  }

  function renderStudyPatientAssignment(state = readState()) {
    const body = $('#studyPatientBody');
    if (!body) return;
    const tagId = cardByHeading('연구 태그 선택')?.querySelector('select')?.value || '';
    if (!state.patients.length) return setTableEmpty(body, 6, '배정할 환자 master가 없습니다.', '먼저 위에서 고정 환자 정보를 등록하세요.');
    body.innerHTML = state.patients.map(p => {
      const assigned = state.studyPatients.some(sp => sp.tagId === tagId && sp.patientId === p.id);
      return `<tr><td><input type="checkbox" data-study-patient="${escapeHtml(p.id)}" ${assigned ? 'checked' : ''}></td><td>${escapeHtml(p.pseudoId || '-')}</td><td>${escapeHtml(p.registrationNo ? '****' + p.registrationNo.slice(-4) : '-')}</td><td>${escapeHtml(p.sex || '-')}</td><td>${escapeHtml(p.birthYear || '-')}</td><td>${assigned ? badge('배정됨','green') : badge('미배정','gray')}</td></tr>`;
    }).join('');
  }

  function saveStudyPatients() {
    const tagId = cardByHeading('연구 태그 선택')?.querySelector('select')?.value || '';
    const stateBefore = readState();
    const tag = stateBefore.tags.find(t => t.id === tagId);
    if (!tag) return toast('연구 태그를 먼저 선택하세요.', 'gray');
    const selected = $$('[data-study-patient]:checked').map(x => x.dataset.studyPatient);
    patchState(s => {
      s.studyPatients = s.studyPatients.filter(sp => sp.tagId !== tagId);
      selected.forEach(pid => s.studyPatients.push({ id: uid('SP'), tagId, tagName: tag.name, patientId: pid, assignedAt: nowText(), assignedBy: USER }));
    });
    renderStudyPatientAssignment();
    setMetricByLabel('선택된 대상자', `${selected.length}명`, selected.length ? '배정됨' : '대기');
    addAudit('study patient assignment', 'patient', `${tag.name}: ${selected.length}명`);
    toast('연구 태그에 대상자를 배정했습니다.');
  }

  function initPatientUpload() {
    const tagSelect = cardByHeading('연구 태그 선택')?.querySelector('select');
    fillTagSelect(tagSelect, '등록된 연구 태그가 없습니다. 먼저 태그를 생성하세요.');
    tagSelect?.addEventListener('change', () => { populatePatientInternalSelects(tagSelect.value); renderStudyPatientAssignment(); renderVariableLibrary(); });
    populatePatientInternalSelects(tagSelect?.value || '');
    const patientSaveBtn = $('#patientMasterSaveBtn');
    const studySaveBtn = $('#studyPatientSaveBtn');
    [patientSaveBtn, studySaveBtn].forEach(removeDisabledAction);
    patientSaveBtn?.addEventListener('click', savePatientMaster);
    studySaveBtn?.addEventListener('click', saveStudyPatients);
    renderPatientMaster();
    renderStudyPatientAssignment();

    const addBtn = cardByHeading('입력할 자료 항목 직접 고르기')?.querySelector('button');
    removeDisabledAction(addBtn);
    addBtn?.addEventListener('click', () => addClinicalVariable());

    const variableSaveBtn = $('#variableSaveBtn');
    const variableResetBtn = $('#variableResetBtn');
    variableSaveBtn?.addEventListener('click', () => saveVariableDefinition());
    variableResetBtn?.addEventListener('click', () => resetVariableForm());
  ...
  function initStatisticalAnalysis() {
    const tagSelect = cardByHeading('데이터 선택')?.querySelector('select');
    fillTagSelect(tagSelect, '등록된 연구 태그가 없습니다.');
    tagSelect?.addEventListener('change', () => { populateAnalysisOptions(); updateAnalysisPreview(); });
    populateAnalysisOptions();
    $$('input[name="method"]').forEach(r => r.addEventListener('change', updateAnalysisPreview));
    $$('select, input', cardByHeading('x축·y축·변수 설정') || document).forEach(el => el.addEventListener('input', updateAnalysisPreview));
    $$('input', cardByHeading('그래프 스타일') || document).forEach(el => el.addEventListener('input', updatePlotStyle));
    const runBtn = $('.preview-card button.btn.teal');
    removeDisabledAction(runBtn);
    runBtn?.addEventListener('click', saveAnalysisJob);
    updateAnalysisPreview();
    updatePlotStyle();
    renderSignalTrajectoryPanel($('#analysisTrajectoryPreview'), [], '', {title:'분석 그래프 미리보기', subtitle:'x축/y축/그룹과 실제 데이터가 연결되면 이 영역에 궤적 또는 분석 그래프가 생성됩니다.', emptyText:'아직 표시할 실제 분석 데이터가 없습니다.', badge:'preview'});
  }

  function populateAnalysisOptions() {
    const state = readState();
    const tagId = cardByHeading('데이터 선택')?.querySelector('select')?.value || '';
    const axes = state.axes.filter(a => !tagId || a.tagId === tagId);
    const variables = [
      ...state.variables.filter(v => !tagId || v.tagId === tagId),
      ...(state.variableDefinitions || []).filter(v => !tagId || !v.tagId || v.tagId === tagId).map(v => ({...v, id: `def:${v.id}`, status: '변수 사전'}))
    ];
    const setting = cardByHeading('x축·y축·변수 설정');
    const selects = $$('select', setting);
    if (selects[0]) selects[0].innerHTML = option('', 'x축 변수 선택') + option('elapsed_time', '시간') + axes.map(a => option(`axis:${a.id}`, a.name)).join('') + option('custom', '사용자 지정 변수');
    if (selects[1]) selects[1].innerHTML = option('', 'y축 변수 선택') + variables.map(v => option(`var:${v.id}`, v.name)).join('') + option('vital_numeric', 'Vital numeric variable') + option('lab_value', 'Lab value') + option('clinical_variable', 'Clinical variable') + option('outcome_variable', 'Outcome variable');
    if (selects[2]) selects[2].innerHTML = option('', '그룹 없음') + axes.map(a => option(`axis:${a.id}`, a.name)).join('') + option('clinical_condition', '임상변수 기준') + option('custom_condition', '직접 조건식');
  }
  ...
  function updateAnalysisPreview() {
    ...
    const tagId = cardByHeading('데이터 선택')?.querySelector('select')?.value || '';
    const tag = state.tags.find(t => t.id === tagId);
    const method = $('input[name="method"]:checked')?.closest('.mini-item')?.querySelector('strong')?.textContent.trim() || '';
    const setting = cardByHeading('x축·y축·변수 설정');
    const selects = $$('select', setting);
    const x = selectedText(selects[0]) || '미선택';
    const y = selectedText(selects[1]) || '미선택';
    const group = selectedText(selects[2]) || '그룹 없음';
    const covariates = setting?.querySelector('input')?.value.trim() || '';
    const previewRows = $$('.preview-card .stat-row');
    const vitals = state.vitalFiles.filter(f => !tagId || (f.tagIds || []).includes(tagId));
    const vars = [
      ...state.variables.filter(v => !tagId || v.tagId === tagId),
      ...(state.variableDefinitions || []).filter(v => !tagId || !v.tagId || v.tagId === tagId)
    ];
    if (previewRows[0]) previewRows[0].querySelector('strong').textContent = '0명';
    if (previewRows[1]) previewRows[1].querySelector('strong').textContent = `${vitals.length}개`;
    if (previewRows[2]) previewRows[2].querySelector('strong').textContent = `${vars.length} variables`;
    if (previewRows[3]) previewRows[3].querySelector('span:last-child').outerHTML = badge(x, x === '미선택' ? 'gray' : 'teal');
    if (previewRows[4]) previewRows[4].querySelector('span:last-child').outerHTML = badge(y, y === '미선택' ? 'gray' : 'teal');
    if (previewRows[5]) previewRows[5].querySelector('span:last-child').outerHTML = badge(method || '미선택', method ? 'blue' : 'gray');
    const codebox = $('.preview-card .codebox');
    if (codebox) codebox.textContent = `tag = ${tag?.name || '미선택'}\nmethod = ${method || '미선택'}\ny ~ ${x}${group && group !== '그룹 없음' ? ' + group(' + group + ')' : ''}${covariates ? ' + ' + covariates : ''}\n주의: 실제 p-value 계산은 분석 엔진/API 연결 후 수행됩니다.`;
    const axisLabels = $$('.axis-config span');
    if (axisLabels[0]) axisLabels[0].textContent = `x축: ${x}`;
    if (axisLabels[1]) axisLabels[1].textContent = `y축: ${y}`;
    if (axisLabels[2]) axisLabels[2].textContent = `색상: ${group}`;
    renderSignalTrajectoryPanel($('#analysisTrajectoryPreview'), [], '', {title:'분석 그래프 미리보기', subtitle:`x축 ${x} · y축 ${y} · 색상 ${group} · 방법 ${method || '미선택'}`, emptyText:'실제 데이터와 분석 엔진이 연결되면 선택 조건에 맞는 궤적이 표시됩니다.', badge:'preview'});
  }

  function updatePlotStyle() {
    const card = cardByHeading('그래프 스타일');
    if (!card) return;
    const inputs = $$('input', card);
    const pointSize = Number(inputs[0]?.value || 7.5);
    const lineWidth = Number(inputs[1]?.value || 2.2);
    const ciAlpha = Number(inputs[2]?.value || 0.14);
    $$('circle', card).forEach(c => { if (!Number.isNaN(pointSize)) c.setAttribute('r', String(pointSize)); });
    $$('polyline', card).forEach(l => { if (!Number.isNaN(lineWidth)) l.setAttribute('stroke-width', String(lineWidth)); });
    $$('.lmm-panel', card).forEach(panel => panel.style.setProperty('--ci-alpha', String(ciAlpha)));
  }
  ...
  function saveAnalysisJob() {
    ...
    const setting = cardByHeading('x축·y축·변수 설정');
    const selects = $$('select', setting);
    const x = selectedText(selects[0]);
    const y = selectedText(selects[1]);
    if (!tag || !method || !x || !y) return toast('연구 태그, 분석 방법, x축, y축을 모두 선택하세요.', 'gray');
    const analysis = {
      id: uid('ANALYSIS'),
      name: `${tag.name}_${method}_${nowText()}`,
      tagId,
      tagName: tag.name,
      method,
      x,
      y,
      group: selectedText(selects[2]) || '그룹 없음',
      status: '설정 저장됨',
      createdAt: nowText(),
      createdBy: USER
    };
    patchState(s => s.analyses.unshift(analysis));
    renderAnalysisResultTable(analysis);
    addAudit('analysis run', 'analysis_job', analysis.name);
    toast('분석 설정을 저장했습니다. 실제 통계 계산은 백엔드 분석 엔진 연결 후 수행됩니다.');
  }

  function renderAnalysisResultTable(analysis) {
    const body = cardByHeading('변수별 분석 결과 상세')?.querySelector('tbody');
    if (!body) return;
    body.innerHTML = `<tr><td>${escapeHtml(analysis.y)}</td><td>${escapeHtml(analysis.method)}</td><td>${escapeHtml(analysis.group)}</td><td>계산 대기</td><td>계산 대기</td><td>계산 대기</td><td>${badge('분석 엔진 필요', 'yellow')}</td><td>설정은 저장되었고, p-value는 실제 데이터/분석 API 연결 후 계산됩니다.</td></tr>`;
  }
  ...
  function saveResearchProject() {
    ...
    const irb = inputs[2]?.value.trim() || '';
    const objective = $('textarea', card)?.value.trim() || '';
    const status = $('select', card)?.value || '준비 중';
    ...
    patchState(s => {
      ...
      s.projects.unshift({ id: projectId, title, topic, objective, irb, status, createdAt: nowText(), createdBy: USER });
      if (!s.tags.some(t => t.name === tagKeyword)) {
        s.tags.unshift({ id: uid('TAG'), projectId, name: tagKeyword, status, color: '#0b8f8a', description: objective || topic, irb, matchUnit: '복합 단위', createdBy: USER, createdAt: nowText(), updatedAt: nowText() });
      }
    });
    $$('input, textarea', card).forEach(el => el.value = '');
    renderResearchProjects();
    addAudit('research project create', 'research_project', title);
  ...
  function renderResearchProjects(state = readState()) {
    ...
    if (!body) return;
    if (!state.projects.length) return setTableEmpty(body, 7, '아직 생성된 연구가 없습니다.', '연구 제목과 목적을 저장하면 해당 연구를 구분하는 태그가 함께 생성됩니다.');
    body.innerHTML = state.projects.map(p => {
      const tag = state.tags.find(t => t.projectId === p.id || t.name === p.title.replace(/\s+/g, '_'));
      const subjectCount = state.studyPatients.filter(sp => sp.tagId === tag?.id).length;
      return `<tr><td>${escapeHtml(p.title)}</td><td>${escapeHtml(p.topic || '-')}</td><td>${escapeHtml(p.irb || '-')}</td><td>${badge(p.status || '준비 중','blue')}</td><td>${tag ? badge(tag.name,'teal') : badge('태그 없음','gray')}</td><td>${subjectCount}명</td><td><a class="btn ghost" href="patient-upload.html">대상자/자료 배정</a></td></tr>`;
    }).join('');
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (page === 'tag-manager.html') initTagManager();
    if (page === 'patient-upload.html') initPatientUpload();
    if (page === 'tag-assignment.html') initTagAssignment();
    ...
    if (page === 'patient-matching.html') initPatientMatching();
